...
public class FilterByCoverage implements ITestSelector{
	...
	public void generateOverallScoreReport(StringBuilder stringBuilder) {
		...
		for (int tsCoverageIndex=0; tsCoverageIndex< tsCoverageReport.getObjectCoverageStatus().size(); tsCoverageIndex++) {
			if (tsCoverageReport.getObjectCoverageStatus().get(tsCoverageIndex) != TDLCoverageUtil.NOSTATUS) {
				ElementCoverageStatus entry = new ElementCoverageStatus();
				entry.elementName = TDLTestResultUtil.getInstance().eObjectLabelProvider(tsCoverageReport.getObjects().get(tsCoverageIndex));
				entry.coverageStatus = tsCoverageReport.getObjectCoverageStatus().get(tsCoverageIndex);
