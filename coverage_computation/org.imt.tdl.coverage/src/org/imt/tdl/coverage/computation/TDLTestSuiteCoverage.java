...
public class TDLTestSuiteCoverage {
	...
	public TestCoverageReport getTsObjectCoverageRepot() {
		TestCoverageReport report = getTsCoverageRepot(TDLCoverageUtil.DSLSPECIFICCOVERAGE);
		if (report == null) {
			report = getTsCoverageRepot(TDLCoverageUtil.TRACEBASEDCOVERAGE);
		}
		return report;
	}
	...
	public TestCoverageReport getTsCoverageRepot(String reportTitle) {
		return coverageReports.stream()
				.filter(r -> r.getReportTitle().startsWith(reportTitle)).findFirst().orElse(null);
	}
