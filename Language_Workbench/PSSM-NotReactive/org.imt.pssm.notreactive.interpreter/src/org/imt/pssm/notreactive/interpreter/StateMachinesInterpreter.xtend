package org.imt.pssm.notreactive.interpreter
...
import static extension org.imt.pssm.notreactive.interpreter.BehaviorAspect.*
import static extension org.imt.pssm.notreactive.interpreter.ConstraintAspect.*
import static extension org.imt.pssm.notreactive.interpreter.RegionAspect.*
import static extension org.imt.pssm.notreactive.interpreter.StateAspect.*
import static extension org.imt.pssm.notreactive.interpreter.StateMachineAspect.*
import static extension org.imt.pssm.notreactive.interpreter.TransitionAspect.*
import static extension org.imt.pssm.notreactive.interpreter.VertexAspect.*
...
class CustomSystemAspect{
	...
	def void main(){
		...
		_self.eventOccurrences.forEach[e | _self.statemachine.eventOccurrenceReceived(e)]
	}
}
...
//			println(_self.name + "(" + _self.entry.name + ")" + if (eventOccurrence !== null) {eventOccurrence.parameters} else {""})
			...
			(_self.container.containingStateMachine.eContainer as CustomSystem).performedBehaviors.add(_self.entry)
			if (_self.hasCompleted) {
				_self.complete
			}
		}
...
//			println(_self.name + "(" + _self.doActivity.name + ")" + if (eventOccurrence !== null) {eventOccurrence.parameters} else {""})
			...
			(_self.container.containingStateMachine.eContainer as CustomSystem).performedBehaviors.add(_self.doActivity)
			if (_self.hasCompleted) {
				_self.complete
			}
		}
...
//			println(_self.name + "(" + _self.exit.name + ")" + if (eventOccurrence !== null) {eventOccurrence.parameters} else {""})
			...
			(_self.container.containingStateMachine.eContainer as CustomSystem).performedBehaviors.add(_self.exit)
		}
	}
...
class TransitionAspect {
	...
	def void fire(EventOccurrence eventOccurrence) {
		...
		if (_self.effect !== null) {
			...
			(_self.container.containingStateMachine.eContainer as CustomSystem).performedBehaviors.add(_self.effect)
